...
async function getLastActionText() {
  const { lastActionText } = await chromePromise.storage.local.get(
    "lastActionText"
  );
  return lastActionText || "";
}
async function getEnabledSugestions() {
  let { disabledActions } = await chromePromise.storage.local.get(
    "disabledActions"
  );
...
async function getSwitchTabSugestions() {
  const allTabs = await chromePromise.tabs.query({
    windowId: chromePromise.windows.WINDOW_ID_CURRENT
  });
...
function handleKeydown(e) {
  switch (e.which) {
    case 40: {
      ...
      const newSuggestion =
        highlightedSuggestion.nextSibling ||
    ...
    suggestionTag.onclick = async function() {
      try {
        await chromePromise.storage.local.set({
          lastActionText: document.getElementById("command").value
        });
...
async function fuzzySearch() {
  ...
  const searchResults = fuzzy
    ...
    .map(el => {
      ...
      const delimited = el.string.replace(
        new RegExp(options.post + options.pre, "g"),
        ""
      );
...
async function initCommander() {
  ...
  document.getElementById("command").value = await getLastActionText();
  ...
  const shortcutEl = document.querySelector("#shortcut a");
  shortcutEl.innerText = mainCommand.shortcut;
  shortcutEl.onclick = async function() {
    await chromePromise.tabs.create({ url: "chrome://extensions/shortcuts" });
  };
}
